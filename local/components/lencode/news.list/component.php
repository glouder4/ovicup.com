...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	$rsElement = CIBlockElement::GetList($arSort, array_merge($arFilter , $arrFilter), false, (!$GLOBALS['cupFilter']['PROPERTY_ROLE'])?$arNavParams:false, $shortSelect);
	$nn = 1;
	...
	{           
        ...
        $keep = 0;
        $miss = 0;
        $db_props = CIBlockElement::GetProperty(2, $id, array("sort" => "asc"), Array("CODE"=>"TEAM"));
        if($ar_props = $db_props->Fetch()) {
            $team = $ar_props["VALUE"];
        }
        if($team) {
        $db_props = CIBlockElement::GetProperty(1, $team, array("sort" => "asc"), Array("CODE" => "CUP"));
        if ($ar_props = $db_props->Fetch()) {
            if ($ar_props['VALUE'] == $GLOBALS['cupFilter']['PROPERTY_CUP']) {
                $res = CIBlockElement::GetList(array(), array("IBLOCK_ID" => 10, "PROPERTY_GAME_OVER" => 21, array("LOGIC" => "OR", array("PROPERTY_SRTUCTURE1" => $row['ID']), array("PROPERTY_SRTUCTURE2" => $row['ID']))), false, false, array("ID"));
                ...
                $arResult['ITEMS'][$id]['SCORED'] = $scored;
                $arResult['ITEMS'][$id]['ASSIST'] = $assist;
                $arResult['ITEMS'][$id]['KEEP'] = $keep;
                $arResult['ITEMS'][$id]['MISSED'] = $miss;
                if ($arParams['FILTER_NAME'] == "Sniper") {
                    $arResult['ITEMS'][$id]['SCORE'] = $scored;
                } else {
                    $arResult['ITEMS'][$id]['SCORE'] = $scored + $assist;
                }
            }
        }
        } else {
